...
**Сервер:** `139.100.207.10`
**Подключение:** `ssh mynamemyway@139.100.207.10`
...
* **Команда:**
```bash
ssh mynamemyway@139.100.207.10
```

### 3.1. Инфраструктура и DNS

1. **DNS:** Убедись, что А-запись `card.rassada1.ru` → `139.100.207.10` активна (`ping card.rassada1.ru`).
...
```bash
mkdir -p /home/mynamemyway/projects
```
...
```bash
scp .env.production mynamemyway@139.100.207.10:/home/mynamemyway/projects/antoshkin-pwa-card/.env
```
